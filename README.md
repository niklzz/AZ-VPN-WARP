# V2 — AntiZapret VPN + полный VPN на сервере в России

Форк [AntiZapret-VPN](https://github.com/GubernievS/AntiZapret-VPN) для установки на сервер **внутри России**,
который разводит трафик по трём путям вместо одного.

| путь | что попадает | куда уходит |
|---|---|---|
| **байпас** | всё, что не в списках | в туннель не входит, идёт через провайдера клиента |
| **заграница** | список АнтиЗапрета (реестр РКН + свои правки) | аплинк AmneziaWG на зарубежный сервер |
| **WARP** | свой список `config/warp-hosts.txt` | Cloudflare WARP |

Зачем третий путь: сервисы вроде ChatGPT или Spotify не заблокированы в России, но сами не пускают
российские IP-адреса. Их достаточно завернуть в WARP, гонять через зарубежный сервер незачем.

**Домен, попавший в оба списка, уходит в WARP** — свой список приоритетнее общего реестра.
Нужно обратное для конкретного домена — убери его из `warp-hosts.txt`.

### Что нужно от зарубежного сервера

Ничего, кроме одного клиентского профиля. Это может быть любой WireGuard/AmneziaWG-сервер, поднятый
чем угодно (`wireguard-install`, AmneziaVPN, wg-easy) — логики АнтиЗапрета там нет, менять на нём нечего.
Единственное требование: он NAT'ит своих пиров в интернет, что делает любой нормальный инсталлятор.

Вся логика — kresd, `proxy.py`, подменные IP, DNAT, списки — живёт на российском сервере.
От апстрима берутся только **сами списки заблокированного**: их ведёт автор оригинального проекта
вместе с комьюнити, и поддерживать их в одиночку нереально.

### Отличия от оригинала

- аплинк `az` до зарубежного сервера, отдельная таблица маршрутизации 13337
- второй подменный диапазон и вторая цепочка `V2-WARP-MAPPING` под WARP-список
- выбор пути по метке `fwmark` в `mangle PREROUTING`, а не по адресу источника
- WARP перестал быть выходом всего АнтиЗапрета и обслуживает только свой список
- апстримы `kresd@2` и регистрация Cloudflare WARP ходят через аплинк — из России они недоступны
- код обновляется из этого форка, списки — по-прежнему из репозитория оригинала

Полный VPN (файлы `vpn-*`) сохранён без изменений: весь трафик через российский сервер как прокси.

---

| У автора оригинального проекта есть собственный VPN-сервер. Вы можете приобрести готовое подключение к AntiZapret VPN или получить помощь в настройке своего сервера или роутера. Все подробности и стоимость [тут](https://t.me/antizapret_vpn/4) |
|------------------|
...
2. На зарубежном сервере создать клиентский профиль и скопировать его в `/root` российского сервера **до запуска установщика** — без него установка не начнётся. Имя файла любое: установщик сам найдёт `*.conf` в `/root`, а если их несколько — спросит, какой из них аплинк:
```
scp foreign-server-client.conf root@СЕРВЕР:/root/
```
3. Скопировать установщик из этого форка и запустить его под root:
```
scp setup.sh root@СЕРВЕР:/root/setup.sh
bash /root/setup.sh
```
4. Изменить настройки (или нажимать Enter для выбора значения по умолчанию):
	- Установить патч для обхода блокировки протокола OpenVPN (только для UDP соединений)
	- Включить OpenVPN DCO
	- Включить Cloudflare WARP для списка `config/warp-hosts.txt`
	- Указать токен GitHub, если форк приватный (ввод скрыт)
	- Включить Cloudflare WARP для исходящего трафика обычного VPN
...
5. Дождаться перезагрузки сервера и скопировать файлы подключений (*.ovpn и *.conf) с сервера из подпапок /root/antizapret/client (например через MobaXterm, FileZilla или WinSCP)
6. Заполнить список для WARP в `/root/antizapret/config/warp-hosts.txt` и применить его:
```
/root/antizapret/doall.sh host
```
7. Установить дополнения:
...
### 7. Добавить свои сайты в список для Cloudflare WARP
```
nano /root/antizapret/config/warp-hosts.txt
```
Домены из этого файла идут через WARP, а не через зарубежный сервер\
Нужно для сервисов, которые не заблокированы в России, но сами не пускают российские IP-адреса\
Добавлять нужно только домены, например:\
chatgpt.com\
spotify.com\
После этого нужно обновить списки
```
/root/antizapret/doall.sh host
```
Этот список имеет приоритет над списком АнтиЗапрета: домен, попавший в оба, пойдет через WARP

### 8. Проверить, каким путем идет домен
```
dig +short @127.1.1.1 example.com
```
Адрес из диапазона подменных IP АнтиЗапрета (198.18.0.0/15 по умолчанию) - домен пойдет через зарубежный сервер\
Адрес из диапазона подменных IP WARP (10.30.0.0/15 по умолчанию) - домен пойдет через WARP\
Реальный адрес - домен пойдет мимо VPN, через провайдера клиента
