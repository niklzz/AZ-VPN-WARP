...
2. На зарубежном сервере создать клиентский профиль и скопировать его на российский сервер **до запуска установщика** — без него установка не начнётся:
```
scp foreign-server-client.conf root@СЕРВЕР:/root/v2-uplink.conf
```
