...
Поковыряв WARP и выяснив, что если выходная нода WARP является HEL (обычно при коннекте из СПБ), то мы получаем следующие плюсы:
...
- Зарубежный сервер нужен только для сайтов из гео-блока, что позволяет, в некоторых случаях, от него отказаться полностью.
