# V2 — AntiZapret VPN на сервере в России

Форк [AntiZapret-VPN](https://github.com/GubernievS/AntiZapret-VPN) для установки на сервер
**внутри России**, который разводит трафик по трём путям вместо одного.

В чем разница с оригинальным проектом и зачем нужен еще нужен еще один "велосипед"?
Поковыряв WARP и выяснив, что если выходная нода HEL (обычно при коннекте из СПБ), то мы получаем следующие плюсы:
...
- соединение РФ-РФ не так жестко блокируется ТСПУ, как РФ-ЗАРУБЕЖ. Клиент может ходить, хоть по чистому WG
- получить сервер в РФ проще, не нужны иностранные карты
- т.к весь список AZ завернут в WARP, сильно выше скорость/отзывчивость сайтов в целом
- зарубежный сервер все еще нужен, но требования к его ресурсам и пропускной способности на порядки ниже т.к теперь он нужен только для сайтов из гео-блока

Что доработано:
- для az.conf теперь три маршрута для трафика
- интегрирован генератор WARP соединений с обфускацией AmneziaWG
- интегрирован "сканер" WARP серверов. Показывает доступные выходные ноды с вашего сервера и выбирает ноду с лучшим пингом

## Поснения по трем маршрутам в AZ
```
...
От апстрима берутся только **сами списки заблокированного**: их ведёт автор оригинального проекта
вместе с комьюнити, поддерживать их в одиночку нереально.
