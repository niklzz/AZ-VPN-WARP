...
**WARP-ветке нужен профиль с обфускацией — `/root/v2-warp.conf`.** Он кладётся рядом с профилем аплинка,
`up.sh` санирует его в `/etc/amnezia/amneziawg/warp-antizapret.conf` (вырезает `DNS`, добавляет
`Table = 13335` и `ip rule` по метке) и поднимает через **`awg-quick`**. Туннель идёт **байпасом**,
не в аплинк: Cloudflare выдаёт адрес той страны, откуда пришли пакеты, поэтому прямой выход из России
даёт российский Cloudflare-адрес — а уведёшь в аплинк, получишь адрес страны аплинка, и смысл ветки теряется.

Без профиля `up.sh` откатывается на прежний путь — сам регистрируется в Cloudflare и поднимает
**чистый WireGuard**. Из России это не работает: handshake чистого WG опознаётся ТСПУ и режется
(симптом: интерфейс UP, `proxy.py` живой, а `wg show warp-antizapret` растёт только по `sent`,
`0 B received`). Перебор портов (2408/500/4500/1701) помогает лишь вероятностно. Поэтому обе стороны
обязаны говорить на AmneziaWG, а односторонне обфускацию не включить — `S1..S4`/`H1..H4` меняют формат
заголовков и нужны обоим концам.

Регистрация же (`api.cloudflareclient.com`) наоборот идёт **только через аплинк** — из России домен
не отвечает вовсе. Поэтому в ветке самостоятельной регистрации `--interface $UPLINK_INTERFACE` обязателен.

Диагностика: `up.sh` ждёт именно handshake, а не код возврата `wg-quick`/`awg-quick` — тот рапортует
об успехе и на молчащем endpoint. В журнале ищи `obfuscated profile connected` либо `but no handshake`.
