...
- **Профиль аплинка обязателен до старта установки.** Имя файла не важно: нет `/root/v2-uplink.conf` —
  установщик берёт любой валидный WG/AWG-профиль из `/root` (`[Peer]` + `Endpoint`), кроме `v2-warp.conf`
  (по формату он от аплинка не отличим); нашлось несколько — спрашивает номер, ни одного — код 11.
...
**При установке узел не выбирается вовсе**: скан требует поднятых `az` и `warp-antizapret`, а до ребута их
нет. Первый старт берёт тот endpoint, который выдала регистрация Cloudflare, — узел какой достанется.
Выбор узла — только руками через `warp.sh` после ребута; вопрос установщика лишь пишет `WARP_NODE`.

`WARP_NODE` — список через запятую в порядке предпочтения (`HEL,ARN,FRA`), **используется только
автофолбэком** (`warp.sh auto` из `up.sh`), когда handshake не случился. Автоматика идёт по нему
сверху вниз и **за пределы списка не выходит**, поэтому пустой список безопаснее непроверенного:
пусто — берётся лучший найденный узел, а недоступный `HEL` в списке оставит ветку лежать. Какой узел обслужит anycast-адрес, решает сеть:
...
```bash
# Установка (на сервере, под root). Профиль аплинка должен уже лежать в /root, имя любое
scp foreign-client.conf root@СЕРВЕР:/root/
