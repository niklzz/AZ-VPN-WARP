...
**Одной регистрации мало: сам WARP-туннель тоже надо прибить к аплинку.** Cloudflare отдаёт endpoint
именем (`engage.cloudflareclient.com:2408`), и без маршрута `wg-quick` погнал бы UDP по `main` — напрямую
из России, где handshake не случается (симптом: `wg show warp-antizapret` растёт только по `sent`,
`0 B received`, а интерфейс при этом UP и `proxy.py` живой). Поэтому `up.sh` резолвит имя через прибитый
к аплинку `1.1.1.1`, **записывает полученный IP прямо в конфиг** (иначе `wg-quick` возьмёт из A-записи
другой адрес мимо маршрута) и уводит этот IP в аплинк. Проверять после рестарта:
в `/etc/wireguard/warp-antizapret.conf` должен стоять `Endpoint = <IP>:2408`, а не имя.
